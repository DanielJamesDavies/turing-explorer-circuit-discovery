...
class MlpSparseExpansion(DiscoveryMethod):
    """
    MLP-targeted variable-depth co-activation expansion with full attn/resid passthrough.

    The circuit is partitioned into two logically separate parts:

    Sparse MLP part (co-activation expansion)
    -----------------------------------------
    Starting from an MLP seed latent, the algorithm expands through the
    pre-computed co-activation graph for ``len(coact_depth)`` levels, retaining
    only MLP-kind latents at every level.  attn and resid neighbors are skipped.

      Depth 0  — the seed MLP latent (role="seed").
      Depth 1  — top ``coact_depth[0]`` MLP neighbors of the seed (role="hop1").
      Depth 2  — top ``coact_depth[1]`` MLP neighbors of each depth-1 node (role="hop2").
      ...and so on for each entry in coact_depth.

    Examples:
      coact_depth=[32]        → seed + up to 32 direct MLP neighbors
      coact_depth=[32, 32]    → previous + up to 32×32 second-level neighbors
      coact_depth=[32, 16, 8] → three levels, narrowing at each depth

    Live attn/resid part (passthrough)
    -----------------------------------
    A single no-grad forward pass under SAEGraphInstrument captures every latent
    that fires for any attn or resid component.  These are added with
    role="passthrough" and no edges.  CircuitPatcher produces ``patched = x``
    for those components, contributing zero faithfulness loss.

    Args:
        coact_depth:       List of per-depth top-N limits, e.g. [32, 32].
                           Length determines expansion depth; each value is the
                           max MLP neighbors added per node at that depth.
        min_faithfulness:  Minimum faithfulness to accept a circuit.
        min_active_count:  Minimum lifetime firing count for an MLP candidate.
    ...
    def _expand_mlp_neighbors(
        ...
        ``limit`` co-activation neighbors of (comp_idx, latent_idx) that are:
          1. kind == "mlp"          ← the key filter for this algorithm
    ...
    def _capture_passthrough_nodes(
        ...
        collect every latent that fires for any attn or resid component.
    ...
    def discover(self, seed_comp_idx: int, seed_latent_idx: int) -> Optional[Circuit]:
        """Build an MLP-sparse + attn/resid-passthrough circuit from the seed feature."""
        logger = CircuitLogger(seed_comp_idx, seed_latent_idx, "mlp_sparse_expansion")
        try:
    ...
    def _discover(
        ...
        if seed_kind != "mlp":
            logger.reject(
                f"seed kind '{seed_kind}' is not 'mlp'; MlpSparseExpansion requires an MLP seed"
            )
            return None

        circuit = Circuit(name=f"MlpSparse_S{seed_comp_idx}_{seed_latent_idx}")
        ...
        # ----------------------------------------------------------------
        # Variable-depth MLP co-activation expansion (BFS)
        # frontier holds (comp_idx, latent_idx, key) for the current depth
        # ----------------------------------------------------------------
        # Each entry in coact_depth drives one level of BFS expansion.
        ...
        for depth_idx, n_coacts in enumerate(self.coact_depth):
            ...
            for parent_comp, parent_lat, parent_key in frontier:
                for n_comp, n_lat, w in self._expand_mlp_neighbors(
            ...
            logger.stage(
                f"depth-{depth_idx + 1} MLP expansion",
                len(circuit.nodes), len(circuit.edges),
                note=f"{len(frontier)} nodes expanded, {n_added} new MLP nodes added",
            )
            frontier = next_frontier

        if len(circuit.nodes) <= 1:
            logger.reject("no MLP neighbors found (all below activity filter or empty)")
            return None

        # ----------------------------------------------------------------
        # Passthrough: add every active attn/resid latent from probe pass
        # ----------------------------------------------------------------
        ...
        logger.stage(
            "attn/resid passthrough", len(circuit.nodes), len(circuit.edges),
        ...
        logger.note(f"EVAL_KIND_LOCAL  target_kinds=('mlp',)  faithfulness={final_f_local:.4f}")
        ...
        circuit.metadata.update({
            ...
            "discovery_method":   "mlp_sparse_expansion",
