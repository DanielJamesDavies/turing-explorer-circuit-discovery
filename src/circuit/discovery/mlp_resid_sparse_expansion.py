...
class MlpResidSparseExpansion(DiscoveryMethod):
    """
    MLP+resid-targeted variable-depth co-activation expansion with attn passthrough.

    Sparse mlp/resid part (co-activation expansion)
    -----------------------------------------------
    Starting from an mlp or resid seed latent, the algorithm expands through the
    pre-computed co-activation graph for ``len(coact_depth)`` levels, retaining
    only mlp or resid kind latents at every level. attn neighbors are skipped.

      Depth 0  - the seed mlp/resid latent (role="seed").
      Depth 1  - top ``coact_depth[0]`` mlp/resid neighbors (role="hop1").
      Depth 2  - top ``coact_depth[1]`` mlp/resid neighbors (role="hop2").
      ...and so on for each entry in coact_depth.

    Live attn part (passthrough)
    ----------------------------
    A single no-grad forward pass under SAEGraphInstrument captures every latent
    that fires for attn components. These are added with role="passthrough"
    and no edges.
    """
    ...
    def __init__(
        ...
        cfg = config.discovery.mlp_resid_sparse_expansion
        if coact_depth is not None:
            self.coact_depth = list(coact_depth)
        elif cfg.coact_depth is not None:
            self.coact_depth = list(cast(List[int], cfg.coact_depth))
        else:
    ...
    def _expand_mlp_resid_neighbors(
        self,
        comp_idx: int,
        latent_idx: int,
        limit: int,
    ...
    def _capture_passthrough_nodes(
        ...
        """
        Run a no-grad forward pass and collect every latent that fires
        for attn components.
        """
    ...
    def discover(self, seed_comp_idx: int, seed_latent_idx: int) -> Optional[Circuit]:
        """Build an mlp/resid-sparse + attn-passthrough circuit from the seed feature."""
        logger = CircuitLogger(seed_comp_idx, seed_latent_idx, "mlp_resid_sparse_expansion")
        try:
    ...
    def _discover(
        ...
        if seed_kind not in ("mlp", "resid"):
            logger.reject(
                f"seed kind '{seed_kind}' is not in ('mlp', 'resid'); "
                "MlpResidSparseExpansion requires an mlp or resid seed"
            )
            return None

        circuit = Circuit(name=f"MlpResidSparse_S{seed_comp_idx}_{seed_latent_idx}")
        ...
        for depth_idx, n_coacts in enumerate(self.coact_depth):
            ...
            for parent_comp, parent_lat, parent_key in frontier:
                for n_comp, n_lat, w in self._expand_mlp_resid_neighbors(
            ...
            logger.stage(
                f"depth-{depth_idx + 1} mlp/resid expansion",
                len(circuit.nodes),
                len(circuit.edges),
                note=f"{len(frontier)} nodes expanded, {n_added} new mlp/resid nodes added",
            )
            frontier = next_frontier

        if len(circuit.nodes) <= 1:
            logger.reject("no mlp/resid neighbors found (all below activity filter or empty)")
            return None
        ...
        logger.note(
            "EVAL_KIND_LOCAL  target_kinds=('mlp', 'resid')  "
            f"faithfulness={final_f_local:.4f}"
        )
        ...
        circuit.metadata.update(
            {
                ...
                "kind_local_target_kinds": ["mlp", "resid"],
                "sufficiency": final_s,
                ...
                "discovery_method": "mlp_resid_sparse_expansion",
