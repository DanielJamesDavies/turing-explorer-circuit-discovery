...
class ResidSparseExpansion(DiscoveryMethod):
    """
    Resid-targeted variable-depth co-activation expansion with full attn/MLP passthrough.

    Mirrors MlpSparseExpansion exactly, but targets the residual stream kind:

    Sparse resid part (co-activation expansion)
    --------------------------------------------
    Starting from a resid seed latent, the algorithm expands through the
    pre-computed co-activation graph for ``len(coact_depth)`` levels, retaining
    only resid-kind latents at every level.  attn and mlp neighbors are skipped.

      Depth 0  — the seed resid latent (role="seed").
      Depth 1  — top ``coact_depth[0]`` resid neighbors of the seed (role="hop1").
      Depth 2  — top ``coact_depth[1]`` resid neighbors of each depth-1 node (role="hop2").
      ...and so on for each entry in coact_depth.

    Live attn/MLP part (passthrough)
    ----------------------------------
    A single no-grad forward pass under SAEGraphInstrument captures every latent
    that fires for any attn or mlp component.  These are added with
    ...
    Args:
        coact_depth:       List of per-depth top-N limits, e.g. [32, 32].
                           ...
                           max resid neighbors added per node at that depth.
        min_faithfulness:  Minimum faithfulness to accept a circuit.
        min_active_count:  Minimum lifetime firing count for a resid candidate.
    ...
    def __init__(
        ...
        cfg = config.discovery.resid_sparse_expansion
    ...
    def _expand_resid_neighbors(
        ...
        ``limit`` co-activation neighbors of (comp_idx, latent_idx) that are:
          1. kind == "resid"         ← the key filter for this algorithm
    ...
    def _capture_passthrough_nodes(
        ...
        collect every latent that fires for any attn or mlp component.
    ...
    def discover(self, seed_comp_idx: int, seed_latent_idx: int) -> Optional[Circuit]:
        """Build a resid-sparse + attn/mlp-passthrough circuit from the seed feature."""
        logger = CircuitLogger(seed_comp_idx, seed_latent_idx, "resid_sparse_expansion")
        try:
    ...
    def _discover(
        ...
        if seed_kind != "resid":
            logger.reject(
                f"seed kind '{seed_kind}' is not 'resid'; ResidSparseExpansion requires a resid seed"
            )
            return None

        circuit = Circuit(name=f"ResidSparse_S{seed_comp_idx}_{seed_latent_idx}")
        ...
        # ----------------------------------------------------------------
        # Variable-depth resid co-activation expansion (BFS)
        # ----------------------------------------------------------------
        ...
        for depth_idx, n_coacts in enumerate(self.coact_depth):
            ...
            for parent_comp, parent_lat, parent_key in frontier:
                for n_comp, n_lat, w in self._expand_resid_neighbors(
            ...
            logger.stage(
                f"depth-{depth_idx + 1} resid expansion",
                len(circuit.nodes), len(circuit.edges),
                note=f"{len(frontier)} nodes expanded, {n_added} new resid nodes added",
            )
            frontier = next_frontier

        if len(circuit.nodes) <= 1:
            logger.reject("no resid neighbors found (all below activity filter or empty)")
            return None

        # ----------------------------------------------------------------
        # Passthrough: add every active attn/mlp latent from probe pass
        # ----------------------------------------------------------------
        ...
        logger.stage(
            "attn/mlp passthrough", len(circuit.nodes), len(circuit.edges),
        ...
        logger.note(f"EVAL_KIND_LOCAL  target_kinds=('resid',)  faithfulness={final_f_local:.4f}")
        ...
        circuit.metadata.update({
            ...
            "discovery_method":   "resid_sparse_expansion",
