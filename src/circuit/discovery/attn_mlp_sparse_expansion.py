...
class AttnMlpSparseExpansion(DiscoveryMethod):
    """
    Attn+MLP-targeted variable-depth co-activation expansion with resid passthrough.

    Sparse attn/mlp part (co-activation expansion)
    ----------------------------------------------
    Starting from an attn or mlp seed latent, the algorithm expands through the
    pre-computed co-activation graph for ``len(coact_depth)`` levels, retaining
    only attn or mlp kind latents at every level. resid neighbors are skipped.

      Depth 0  - the seed attn/mlp latent (role="seed").
      Depth 1  - top ``coact_depth[0]`` attn/mlp neighbors (role="hop1").
      Depth 2  - top ``coact_depth[1]`` attn/mlp neighbors (role="hop2").
      ...and so on for each entry in coact_depth.

    Live resid part (passthrough)
    -----------------------------
    A single no-grad forward pass under SAEGraphInstrument captures every latent
    that fires for resid components. These are added with role="passthrough"
    and no edges.
    """
    ...
    def __init__(
        ...
        cfg = config.discovery.attn_mlp_sparse_expansion
    ...
    def _expand_attn_mlp_neighbors(
        self,
        comp_idx: int,
        latent_idx: int,
        limit: int,
    ...
    def discover(self, seed_comp_idx: int, seed_latent_idx: int) -> Optional[Circuit]:
        """Build an attn/mlp-sparse + resid-passthrough circuit from the seed feature."""
        logger = CircuitLogger(seed_comp_idx, seed_latent_idx, "attn_mlp_sparse_expansion")
        try:
    ...
    def _discover(
        ...
        if seed_kind not in ("attn", "mlp"):
            logger.reject(
                f"seed kind '{seed_kind}' is not in ('attn', 'mlp'); "
                "AttnMlpSparseExpansion requires an attn or mlp seed"
            )
            return None

        circuit = Circuit(name=f"AttnMlpSparse_S{seed_comp_idx}_{seed_latent_idx}")
        ...
        for depth_idx, n_coacts in enumerate(self.coact_depth):
            ...
            for parent_comp, parent_lat, parent_key in frontier:
                for n_comp, n_lat, w in self._expand_attn_mlp_neighbors(
            ...
            logger.stage(
                f"depth-{depth_idx + 1} attn/mlp expansion",
                len(circuit.nodes),
                len(circuit.edges),
                note=f"{len(frontier)} nodes expanded, {n_added} new attn/mlp nodes added",
            )
            frontier = next_frontier

        if len(circuit.nodes) <= 1:
            logger.reject("no attn/mlp neighbors found (all below activity filter or empty)")
            return None
        ...
        logger.note(
            "EVAL_KIND_LOCAL  target_kinds=('attn', 'mlp')  "
        ...
        circuit.metadata.update(
            {
                ...
                "kind_local_target_kinds": ["attn", "mlp"],
                "sufficiency": final_s,
                ...
                "discovery_method": "attn_mlp_sparse_expansion",
