...
\begin{abstract}
Circuit discovery in mechanistic interpretability typically explains an output: a logit, a probability, a behavioural metric.
But outputs are a filtered compression of a model's computation.
We instead ask what makes an internal feature fire: the target is a sparse autoencoder (SAE) latent in TuringLLM, a 254M-parameter transformer trained on a documented synthetic corpus.
Signed dependencies are discovered by gradient attribution through the seed latent's encoder pre-activation and by a learned mask; every dependency is validated by intervening on the seed.
The results separate along two criteria.
...
\emph{Faithfulness}, the circuit alone reproducing the seed with all else ablated, is collective and grows with the seed's depth: thousands to tens of thousands of members, each with no measurable effect when removed alone.
Granting members learned scalar gains collapses that cost by one to two orders of magnitude; the compact weighted circuits pass both criteria, while random same-size sets with identically fitted amplitudes fail on every draw.
Membership is partly underdetermined (float rounding exchanges roughly 16\% of members at unchanged quality), so memberships are reported as families, not unique sets.
Behaviour-endpoint circuits, replicated under matched conditions, exert zero measured control over whether a latent fires on every seed tested, and vice versa: the two questions are complementary, not reducible.
The pipeline needs no behavioural task (probe contexts are collected unsupervised from the model's training corpus) and runs across the full SAE bank.
\end{abstract}
...
We can watch what a language model produces, but for most of its behaviour we cannot yet trace how it arrives there.
Individual neurons are hard to interpret: under the superposition hypothesis~\citep{elhage2022toy}, models pack more features than they have dimensions by exploiting high-dimensional geometry, and most neurons are accordingly polysemantic, responding to several distinct concepts at once.
Dictionary learning offers a way out, and sparse autoencoders (SAEs) are its current workhorse: each dense activation vector is decomposed into a handful of active latent features that tend to correspond to coherent semantic or syntactic patterns~\citep{cunningham2024sparse, bricken2023monosemanticity, gao2024scaling, templeton2024scaling}.

But individual latents are only the beginning of an explanation.
Knowing that a feature responds to mathematical notation is interesting; knowing which other features it excites, suppresses, or depends on is where the mechanistic story lives.
Circuit discovery pursues that question, and its methods predominantly root the explanation at an output-level endpoint: a logit, a probability, a behavioural metric.
This holds whether the nodes are architectural components~\citep{conmy2023towards, syed2023attribution} or sparse features~\citep{marks2025sparse, ameisen2025circuit}, and whether the circuit is found by search, attribution, or learned optimisation~\citep{bhaskar2024finding, kharlapenko2025scaling}.
...
The remaining cell of this grid is, to our knowledge, empty: a circuit \emph{optimised} for a single internal feature's activation, and tested by intervening on that feature.
...
\node[meth] (mask) at (5.85,-1.30)
  {\textbf{learned mask}\\ a gate per upstream latent;\\ relaxation of \labelcref{eq:p2star} \ (\S\ref{sec:ablmask})\\[0.36cm]\mbox{}};
...
\end{tikzpicture}
\caption{Overview.
For \emph{any} SAE latent $f$, with no behavioural task, a per-seed problem instance $(f, \mathcal{D}_f, \pi)$ is formed from stored contexts where the seed fires and retrieved contexts where it is silent (\S\ref{sec:problem}).
Discovery runs by gradient attribution (counterfactual and ablation objectives, ranked and cut) and by learned-mask optimisation (a gate per upstream latent).
The discovered circuit is examined under two distinct criteria: \emph{interventional control} (P1), validated by intervening on the seed itself, and \emph{ablation faithfulness} \labelcref{eq:p2}, the measured cost of the circuit reproducing the seed alone under free execution, spanning every upstream site.
...
The answer depends on the test, and the literature tends to conflate two tests under the single word ``circuit''.
\emph{Interventional control} asks for a signed dependency neighbourhood with three operationally distinct roles (\emph{counterfactual activators}, \emph{inhibitors}, and \emph{ablation supports}), discovered by gradient attribution or learned-mask optimisation and validated by direct intervention on the seed's activation; a compact circuit passes it.
\emph{Faithfulness}, the criterion by which prior circuits are scored~\citep{conmy2023towards, marks2025sparse, miller2024transformer}, here instantiated at the latent endpoint, asks that the circuit alone reproduce the seed's activation under free execution, with every non-member removed; far more latents must remain able to fire before a circuit passes that.
Keeping the two criteria apart lets the size of the faithful set be read as a measurement: circuit size, we will argue, reflects computational structure, and is not a quantity to minimise.
...
We discover signed dependencies by contrastive attribution on contexts where the seed is \emph{silent}, attributing through its pre-activation so gradients survive the silence, and validate every dependency by intervening on the seed in both directions.
Negative-context construction is a controlled variable, and random contrasts can beat semantically close hard negatives.
\item \textbf{A learned-mask method for the latent endpoint.}
We move circuit discovery as optimisation~\citep{bhaskar2024finding} to a single latent, with a pre-activation objective forced by Top-K censoring.
The method adds a multi-floor loss training one membership under up to three ablation semantics jointly (a design response to the ablation-sensitivity shown by \citealp{miller2024transformer}), annealed binarisation, one-probe size calibration, and learned per-member amplitudes that return a \emph{weighted circuit}.
\item \textbf{The control/faithfulness decomposition.}
Pinned evaluation asks whether the \emph{right} latents were found; free evaluation asks whether the circuit \emph{reproduces the seed alone}.
The two questions have different answers, and the decomposition is the instrument behind every other result.
...
Interventional control concentrates at $10^{2}$--$10^{3}$ latents; faithfulness needs up to $10^{4}$--$10^{5}$, individually at the measurement floor yet collectively load-bearing, with an irreducible core that grows with seed depth.
Learned amplitudes collapse that cost by one to two orders of magnitude, locating most of it in amplitude-compensable redundancy.
\item \textbf{An evaluation protocol, and a measurement about membership itself.}
Every score names its ablation semantics, a choice that can reorder method rankings.
Membership is partly underdetermined (float rounding exchanges ${\sim}16\%$ of members at unchanged quality), so memberships are reported as families, not sets.
\item \textbf{A controlled boundary with behaviour-endpoint discovery.}
With the attribution estimator held fixed~\citep{marks2025sparse}, circuits explaining a logit exert zero control over whether a chosen latent fires, and vice versa: latent-endpoint discovery is not reducible to behaviour-endpoint discovery.
This is a scope result, measured rather than asserted.
\end{itemize}
...
Sparse feature circuits~\citep{marks2025sparse} moved the nodes to SAE features, with error nodes keeping reconstruction residuals in the graph, position-indexed nodes on templatic data, and an unsupervised pipeline over clustered behaviours; transcoders~\citep{dunefsky2024transcoders} factorise MLP circuits, and attribution graphs~\citep{ameisen2025circuit, lindsey2025biology} instantiate position-indexed graphs per prompt at production scale.
Gradient-times-activation scoring of SAE latents also appears in GradSAE~\citep{shu2025beyond} and, at dictionary-training time, in g-SAEs~\citep{olmo2025features}.
...
\paragraph{Circuit discovery as optimisation.}
Edge Pruning~\citep{bhaskar2024finding} reframed the search as an optimisation problem: continuous masks learned by gradient descent, built on $L_0$-gate and continuous-sparsification machinery~\citep{louizos2018learning, savarese2020winning}, with sparsity targeting in place of a fixed penalty.
\citet{kharlapenko2025scaling} bring the same move to masks over SAE latents.
In both, the training objective is behavioural: a KL divergence or logit difference against the full model.
Our learned mask (\cref{sec:ablmask}) adopts this framing and its gate machinery, and moves the objective to a single latent's pre-activation.
Learned scalars appear elsewhere with different jobs: learned-value ablation learns constants for \emph{removed} components~\citep{li2024optimal}, activation scaling learns steering multipliers for output control~\citep{stoehr2024activation}, and CircuitLasso fits regression weights on feature-to-feature edges~\citep{yin2026scalable}; our amplitudes are gains on \emph{kept} circuit members, trained to reproduce the latent under ablation.

\paragraph{Latent-endpoint attribution.}
The distinction this paper draws is not methodological ingredients (gradients, patching, masks, and sparse dictionaries are common stock) but the root of the explanation, and the internal root is not unexplored.
\citet{ge2024automatically} extract the subgraph activating a chosen SAE feature by thresholded hierarchical attribution; latent attribution at OpenAI traces upstream causes of a chosen latent for debugging~\citep{duprelatour2025debugging}; \citet{dunefsky2024transcoders} reverse-engineer chosen transcoder features from their input circuits in blind case studies; and CircuitLens~\citep{golimblevskaia2025circuit} explains a chosen feature's activation from upstream component interactions at scale.
All four are descriptive: each attributes on prompts where the feature already fires, and none validates the discovered dependencies by intervening on the feature (attribution-graph validation does intervene on features and read effects off other features' activations, but its discovery remains output-rooted;~\citealp{ameisen2025circuit}).
We add attribution where the feature is \emph{silent}, learned-mask discovery in place of thresholds, and intervention validation of every discovered dependency.
The cost of stopping at thresholded attribution is measured in \cref{sec:method-comparison}, including a direct replication of the hierarchical attribution of \citet{ge2024automatically}: at matched compact size, every rank-and-cut arm fails the faithfulness test the optimised weighted circuits pass ($0/21$ against $17/21$).
...
Our membership sets keep the position axis with none of these commitments, using per-position selection unioned over each sequence's causal prefix, for the setting none of the three targets: one reusable circuit over arbitrary non-templatic text (\cref{sec:membership}).
...
Static structure among latents is well documented: co-occurrence and decoder geometry reveal modularity~\citep{li2025geometry}, coactivation-derived components steer behaviour~\citep{deng2026coactivation}, meta-decompositions show latents are not atomic~\citep{leask2025canonical}, cross-layer feature-flow graphs track latents through the forward pass~\citep{laptev2025feature}, and Jacobian SAEs train adjacent dictionaries for sparse computation between them~\citep{farnik2025jacobian}.
But that structure is correlational, geometric, or weight-based, where the relationships discovered here are context-conditioned and intervention-tested (\cref{sec:coact-results}).
...
Experiments use TuringLLM-1.0-254M, a 12-layer decoder-only transformer (residual dimension 1{,}024) trained from scratch on a fully synthetic, documented corpus of ${\sim}2$ billion tokens generated by Phi-3 Mini~\citep{abdin2024phi3}.
The choice is epistemic control: a model whose training data is known is a more tractable subject than one trained on an opaque crawl (architecture, training, and specifications in \cref{app:corpus}, \cref{tab:turinglm}).
...
We formalise the two success criteria as two problems over the same universe: one object, two tests.
...
The paper's central empirical claim is a statement about the two criteria jointly: (P1) admits solutions at $10^{2}$--$10^{3}$ latents, while $n_{\varepsilon}$ sits at $10^{4}$--$10^{5}$ and scales with seed depth (\cref{sec:drivers-closure}).

\paragraph{The semantics are part of the problem.}
Neither problem is well-posed until $(\mathcal{D}_f, \pi)$ is fixed: the ablation is a constitutive choice, not a nuisance parameter~\citep{miller2024transformer, mueller2024missed}, and we measure how sharply it matters: a membership trained to satisfy \labelcref{eq:p2} under one semantics can be exactly infeasible under another (\cref{sec:ablmask}).
...
\end{equation}
with $\pi_0$ zero ablation and $\pi_N$ the negative-context fill, the problem the dual-floor objective trains for.

\paragraph{Weighted circuits.}
% TRI-AMP PASS 2026-08-07. Data: experiments/026-floor-isolation
...
The two variants answer different questions: what must remain at natural gain, versus what must remain at \emph{some} gain.
The sizes of weighted and unweighted solutions are therefore never compared directly; \cref{sec:weighted} measures how far the two answers sit apart.
...
\Cref{sec:underdetermination} shows $\Phi_{\varepsilon}$ is large and non-nested: perturbations at the level of float rounding move the optimiser between members sharing only ${\sim}80\%$ of latents.
A discovered membership is therefore reported as a \emph{sample} from $\Phi_{\varepsilon}$, and only family-invariant quantities (size, faithfulness, the findings about its cost) are treated as properties of the seed.

\paragraph{Solvers.}
The discovery methods of this section are approximate solvers.
Attribution-threshold methods (\cref{sec:cfgrad,sec:modes,sec:membership}) rank $U_f$ and cut; the learned mask solves a differentiable relaxation of \labelcref{eq:p2star}: cardinality relaxes to an $L_1$ penalty on gates $m \in [0,1]^{U_f}$, the constraint on $\phi^{\mathrm{free}}$ (whose seed activation is Top-K censored) relaxes to reproduction of the seed's encoder pre-activation, and the tolerance $\varepsilon$ maps to the penalty weight through a measured size law rather than an assumed one.
...
\]
with $W_e^{(f)}$ the seed's encoder row, $b_e^{(f)}$ its effective bias, and $z(x, t)$ the pre-SAE dense activation.
The pre-activation is defined on every forward pass whether or not $f$ clears Top-K, and is differentiable with respect to all upstream latents through identity passthroughs at each SAE site.
...
\item \textbf{\texttt{ig\_mean}}: integrated gradients along the straight path from the site's mean-ablation floor to the natural activations; this is the $\mathrm{IE}_{\mathrm{ig}}$ node estimator of \citet{marks2025sparse}, which we adopt unchanged and credit as theirs.
\item \textbf{\texttt{restoration}} (ours): iterated greedy re-linearisation, in which each round attributes at the \emph{current} partially restored stream, admits the top scorers, restores them by live re-encoding, and re-linearises, for as many rounds as the seed has upstream sites (\cref{alg:restoration}).
...
We therefore define the discovered object as a latent \emph{membership set}, the object the faithfulness problem \labelcref{eq:p2} optimises over.
...
Terminology, used consistently from here on: a discovered circuit is \emph{one} object, a latent membership, and we do not distinguish kinds of circuit, only the criterion a circuit is examined under: interventional control (P1) or faithfulness \labelcref{eq:p2}.
``Membership set'' is used when the object's type matters; a membership's size at a stated faithfulness is a measured quantity, and \cref{sec:drivers-closure} argues it measures the structure of the computation rather than a defect of the method.
...
A membership set is a subset of the latent dictionary, so rather than thresholding attribution scores it can be \emph{learned} directly, the promised relaxation of \labelcref{eq:p2star}, adopting the optimisation framing and gate machinery of learned-mask circuit discovery~\citep{bhaskar2024finding, kharlapenko2025scaling}.
...
Targeting the pre-activation is forced by the Top-K constraint: the post-Top-K activation is censored at zero, so a loss on it carries no gradient exactly where a mask must learn; this is the same censoring argument that motivates pre-activation attribution in \cref{sec:cfgrad}, now as a training objective.
...
We measure exactly this failure.
A mask trained only under a negative-context fill reaches high fill-faithfulness while its zero-ablation faithfulness is $0.0$ at depth; it learns the delta from the negative baseline, never a sufficient set.
The zero-floored mask has the mirror blindness: sufficient, but never asked what distinguishes firing from a near-identical non-firing context.
The floor is therefore a choice of question: the negative-context floor finds the members that separate firing from almost-firing, and the zero floor finds a set that suffices alone.
For both at once, the intersection problem \labelcref{eq:p2star}, one mask is scored under \emph{both} semantics, with gradients from both accumulating into the same parameters:
\[
    \mathcal{L}
...
% (026-floor-isolation); lambda house value 1e-4 after the retune.
...
The floor is the stronger determinant of which latents are found.

\paragraph{Free amplitudes.}
Each latent optionally carries a learned amplitude $\alpha = \mathrm{softplus}(\psi)$ multiplying its live code behind the gate, initialised at $\alpha = 1$ so training starts exactly at membership-only behaviour; the mask then has free range (off, natural, or rescaled) and returns the weighted circuit of \cref{sec:problem}. We name the two configurations by their floor and their degrees of freedom: \textbf{tri-mask} is the triple-floor mask with amplitudes fixed at $1$, returning a membership; \textbf{tri-amp} adds the free amplitudes and returns a weighted circuit (both are configuration names in the released code).
One guard is required: with amplitudes free, the optimiser can hold a gate just below the keep threshold while inflating the amplitude so the product is unchanged, emptying membership while keeping function intact.
Off-members are therefore charged $\lambda\,(1 - m)\,|\alpha - 1|$, making the escape route exactly as expensive as membership.
...
Intervention scores test interventional control (P1): $\phicf$ sufficiency to impose, $\phisup$ necessity to sustain.
Ablation faithfulness tests \labelcref{eq:p2}: pinned measures node selection, free measures whether the circuit reproduces the seed alone, and each free variant names its ablation semantics $\pi$ (\cref{sec:problem}).
...
Like the intervention scores, $\phi^{\mathrm{free}}_{\pi}$ and $\phi^{\mathrm{pin}}_{\pi}$ are unclipped ratios: success means landing near $1$, and a score far above $1$ is overshoot, not quality.
What remains is fixing $\pi$, and the choice is load-bearing~\citep{li2024optimal, miller2024transformer}.
...
The \emph{grid}: 128 component-balanced random seeds, each processed by the counterfactual, ablation, and hybrid arms under all three negative modes, for 1{,}152 discovery runs, summarised in \cref{tab:grid-descriptives}; it grounds the negative-mode and co-activation findings.
...
The \emph{panels}: three to sixteen depth-spanning seeds per experiment (layers 2--11, all component kinds) under the ablation-faithfulness protocol; these are directional measurements whose magnitudes a depth-stratified full-scale run will refresh, flagged wherever it matters.
...
Seeds accepted is the share of the method's 384 seed--mode discovery runs (128 seeds $\times$ 3 negative modes) whose circuit passed that method's own intervention gate.}
...
Tens of thousands.
The pinned/free decomposition separates the two questions, and on depth-spanning seed panels their answers, the solutions of (P1) and of \labelcref{eq:p2}, come apart; this subsection measures how far.
...
This is the sense in which the cost is \emph{position-local}: no small set of positions carries it, and the score accrues position by position across the causal prefix.
...
These are costs \emph{at natural amplitudes}: \cref{sec:weighted} shows that permitting learned per-member amplitudes collapses them by a factor of ten to a hundred.
...
Yet cutting members in bulk by their own attribution ranking craters function: on recurrence- and bisection-validated memberships, dropping the bottom half by score moves \texttt{free0} from $0.975$ to $0.895$ at layer 2 but from $1.119$ to $0.595$ at layer 8; dropping the bottom 90\% yields $0.635$, $0.000$, $0.222$, and $0.005$ at layers 2, 8, 9, and 10.
The scores being cut are minuscule, $2$--$5 \times 10^{-4}$ in attribution units.
...
\begin{figure}
\centering
% CENTREPIECE v2 (2026-08-09): 3 seeds per band (panel-matched), median
% curves + ghosted seed traces + weighted-circuit markers from
% 029-panel. Data: experiments/028-control-vs-faithfulness-figure/
% curves2.jsonl (collector2.py, render2.py).
\includegraphics[width=\linewidth]{control-vs-faithfulness.pdf}
\caption{Control is sparse; faithfulness is collective; amplitudes collapse its cost.
Cumulative score against members added in attribution order (three seeds per depth band: median curves, individual seeds ghosted; position-aware path attribution, percentile selection).
Pinned faithfulness (node selection) rises steeply within $10^{2}$--$10^{3}$ members, while free faithfulness (the circuit alone, at natural amplitudes) sits near zero until $10^{4}$--$10^{5}$ members, under the mean fill (solid) and zero fill (dashed), each free variant naming its semantics.
The shaded pinned--free gap widens with seed depth.
Diamonds: \emph{weighted circuits} discovered on the same seeds (\cref{sec:weighted}), scored with their learned amplitudes applied under the zero fill, a few hundred members reaching the score the natural-amplitude curve needs $10^{5}$ for.}
\label{fig:drivers-closure}
\end{figure}

\subsection{Weighted Circuits: Amplitudes Index the Faithfulness Cost}
\label{sec:weighted}
% PANEL NUMBERS 2026-08-09: experiments/029-panel (22 seeds,
% layers 2-11, all host kinds, held-out protocol, 124 fitted nulls).
% Calibration history: 026-floor-isolation R14-R21.
...
The free-amplitude mask (\cref{sec:ablmask}) asks the model directly, on a depth-stratified panel of 22 seeds (layers 2--11, all three host kinds) under a held-out protocol: memberships, amplitudes, floors, and pins are fitted on 48 probe contexts and every score below is read on 16 the fit never saw.
The answer is that most of the cost is redundancy.
Weighted circuits at a median of $224$ (shallow) and $471$ (deep) latents pass both zero-fill and mean-fill faithfulness plus necessity on $17$ of $21$ scorable seeds, with median zero-fill faithfulness $0.99$ at \emph{both} depth bands.
Tri-mask memberships under the same acceptance pass on $3$ of $21$, collapsing at depth (median $0.46$ zero-fill, $0.09$ mean-fill) at two to three times the node count.
Held-out generalisation costs reconstruction essentially nothing (median in-sample-to-held-out drift $-0.000$, worst $-0.118$).
The central finding is thereby \emph{indexed}, not overturned: at natural amplitudes, faithfulness still costs far more than interventional control; grant each member a learned scalar and one compact circuit passes both criteria.
The collective tail is largely amplitude-compensable redundancy: many latents each carry a small share of a signal that fewer, rescaled members can carry alone.
This is exactly what the collective-faithfulness picture predicts.
% RUNNING EXAMPLE 2026-08-10: experiments/032-running-example
% (single-seed variant of the panel runner; identical held-out protocol,
% shallow-band config). rows.jsonl + members_<arm>.json.
The running example instantiates the decomposition: the temperature seed's weighted circuit holds $192$ members, reconstructs on held-out contexts (zero-fill $1.12$; mean-fill $1.27$, just past the acceptance band), silences the seed when suppressed, and drives it with the familiar full-budget overshoot ($\phicf^{\alpha} = 1.42$, calibrating to $1.25$ at the quarter budget), while the tri-mask membership for the same seed needs $577$ members and still loses the mean fill ($0.76$).
...
It cannot.
Random same-size sets drawn from the live latent pool, given the identical amplitude-fitting budget on their own members, fail on every one of $124$ draws across the panel.
Their zero-fill faithfulness lands anywhere from $0$ to $4.9 \times 10^{4}$ (median ${\sim}21$) where the discovered circuits hold ${\sim}1$; a free score far from $1$ in either direction is failure (\cref{sec:eval}).
No draw holds both floors, and no draw drives the seed on held-out contrast contexts (maximum $\phicf^{\alpha} = 0.04$ across all draws).
Weighted-circuit faithfulness and drive are both properties of the selected membership, not of the fitting capacity of free coefficients.

\paragraph{The amplitudes carry causal weight.}
Necessity is unchanged by re-weighting (suppression $0.90$--$1.05$ across the panel).
At shallow residual seeds the amplitudes are load-bearing on a task they were never fitted to: injecting members at their clean values drives the seed to only ${\sim}0.6$ on held-out negative contexts, while injecting them at $\alpha \times$ clean values drives it to ${\sim}0.9$--$1.3$.
Drive, unlike reconstruction, is structured by host kind.
Residual-stream seeds drive broadly (with a depth overshoot, next paragraph); MLP-hosted seeds reconstruct perfectly but drive weakly (best $0.88$; two below zero); attention-hosted seeds are bimodal, with one driving at bare clean values and one that cannot be driven at all.
Which latents \emph{can} be puppeted from upstream is itself a per-kind property, and characterising it is open.

\paragraph{The budget is part of the method.}
Sweeping the training budget shows reconstruction faithfulness converges by $100$ steps; everything after buys sparsity.
That sparsity is paid for in drive calibration: on the panel, deep-seed drive overshoots at the full budget (median $\phicf^{\alpha}$ $1.48$) and calibrates at a quarter of it ($1.10$ deep, $1.01$ shallow), at two to three times the circuit size.
The sharpest case replicates held-out.
A layer-9 circuit that reconstructs perfectly yet barely drives at the full budget ($0.17$) \emph{drives} ($0.83$) when discovered at a quarter of it; the calibration sweep shows its drive-carrying members being progressively pruned as training extends ($0.88 \rightarrow 0.57 \rightarrow 0.25$).
...
Amplitude-only training on a frozen membership does not fix the drive overshoot but does inflate amplitudes monotonically; the objective simply does not constrain the drive axis.
Removing the zero-ablation term from training explodes zero-fill faithfulness to $36$--$1{,}681$ at depth while the trained fill stays in band: the sharpest of six demonstrations of what \cref{sec:method-comparison} names the \emph{home-turf effect}, now at the level of coefficients rather than membership.
...
\paragraph{Beyond the model and the SAE family.}
% CROSS-SAE REPLICATION (Top-K): experiments/033-cross-sae-topk —
% EleutherAI/sae-pythia-70m-32k (trained Top-K, k=16, 32,768 latents,
% resid/attn/mlp x 6 layers) on EleutherAI/pythia-70m, wikitext windows,
% held-out 48/16, verified-inactive negatives; 30 seeds after a stated
% weak-seed exclusion (held-out a_pos < 1.0), 146 fitted null draws.
% Scope decision (Daniel, 2026-08-11): dense ReLU dictionaries are out of
% domain for this method — the pre-activation objective is motivated by
% Top-K censoring — so that arm is a scope note in Limitations, not a
% replication claim. Data retained at experiments/030-cross-sae.
Replication on a public model with public \emph{Top-K} dictionaries --- EleutherAI's $k{=}16$, $32{,}768$-latent dictionaries for Pythia-70m, covering attention, MLP, and residual sites at every layer --- reproduces the panel's core results under the same held-out protocol, with negatives \emph{verified} inactive rather than merely retrieved.
Across 30 seeds spanning five layers and all three host kinds, weighted circuits of a median $51$ members hold both fill faithfulnesses and necessity on $22$, against $2$ for tri-mask memberships at $3.4\times$ the size --- the compactness advantage, the collapse of tri-mask on the mean fill, and the growth of circuit size with depth all recur on someone else's dictionaries.
Attention-hosted seeds again reconstruct while barely driving (median $\phicf^{\alpha} = 0.07$), independently reproducing the per-kind drive structure of \cref{sec:weighted}.
Most importantly the null recovers completely: none of $146$ amplitude-fitted random draws passes any criterion, and not one reaches even the lower edge of the faithfulness band (maximum zero-fill score $0.09$).
The reason is measurable in the dictionaries themselves.
A randomly drawn live latent is nonzero at the probe anchor --- and so usable by a fitted amplitude --- on $0.21\%$ of anchors under these Top-$K$ dictionaries ($36$ measurements, range $0.19$--$0.26\%$), so a random member's coefficient almost always multiplies exactly zero.
The null's discriminating power is thus a property of the dictionary's sparsity, and reporting a fitted-coefficient null should state the architecture it was validated on.

\subsection{Method Comparison at Matched Size}
\label{sec:method-comparison}
% DEFINITIVE MATRIX 2026-08-10: experiments/031-matrix (22 panel
% seeds, held-out protocol; mask arms joined from 029-panel).
% FMd vacuity guard applied; @n rows are naive global top-n truncations
% (caveat in prose).

Three discovery families compete for the same object: attribution-threshold arms (\cref{sec:modes,sec:membership}), full-trajectory restoration, and the learned mask (\cref{sec:ablmask}).
\Cref{tab:matrix} is the head-to-head: every arm on the same 22 depth-stratified seeds, the same held-out probes, and the same evaluation semantics, with each attribution arm scored at its natural size and truncated to the weighted circuit's size.
...
\toprule
\textbf{Arm} & \textbf{$n$ (med)} & \texttt{free0} & \texttt{freeM} & $\phicf$ & $\phisup$ & \textbf{pass} \\
\midrule
path attribution (abl)      & 591{,}120 & 0.99 & 0.99 & 1.15 & 1.00 & --- \\
path attribution (cf)       & 591{,}152 & 0.99 & 0.99 & 1.15 & 1.00 & --- \\
restoration (rounds=sites)  & 560{,}077 & 0.99 & 0.97 & 1.14 & 1.00 & --- \\
\midrule
path attr.\ (abl), top-$n$  & 369 & 1.08 & 0.00 & 0.61 & 1.00 & 0/21 \\
path attr.\ (cf), top-$n$   & 369 & 0.99 & 0.00 & 0.56 & 1.00 & 0/21 \\
restoration, top-$n$        & 369 & 4.36 & 0.00 & 0.66 & 0.95 & 0/21 \\
hierarchical attr.\ (replication) & 359 & 0.63 & 0.00 & 0.86 & 1.00 & 0/21 \\
tri-mask                    & 1{,}216 & 0.66 & 0.23 & 0.75 & 1.00 & 3/21 \\
weighted circuit, $100$ steps & 1{,}061 & 0.96 & 1.00 & 1.01 & 1.00 & 12/21 \\
weighted circuit, $400$ steps & 369 & 0.99 & 1.08 & 0.89 & 1.00 & \textbf{17/21} \\
\bottomrule
\end{tabular}
\caption{The cross-method matrix: medians over 22 depth-stratified seeds, every score on held-out probes; weighted-circuit rows apply their amplitudes (the $\alpha$ rows of \cref{tab:semantics}).
``pass'' counts seeds with both fill faithfulnesses in $[0.8, 1.25]$ and $\phisup > 0.9$; one seed's mean-fill denominator is vacuous and excluded.
Top-$n$ rows truncate each attribution ranking to the seed's weighted-circuit size; the hierarchical-attribution row replicates \citet{ge2024automatically} on our sites with its threshold bisected to the same size.}
\label{tab:matrix}
\end{table}

Three results structure the comparison.
First, at natural size every family passes, and the price is the size: all three attribution arms reconstruct held-out at $0.97$--$0.99$ on both fills, at $10^{5}$--$10^{6}$ members, with near-identical per-seed scores --- at the union scale the percentile memberships largely coincide, so method identity matters under compactness pressure, not at the union.
Restoration with its round budget set to the seed's site count is no longer the expensive option (it is the cheapest attribution arm here, median $33$s discovery against $82$--$87$s), resolving the earlier deep failures as a rounds ceiling (\cref{app:mode-studies}).
Second, under compactness pressure the families separate absolutely: truncated to the weighted circuits' size, every attribution arm passes $0/21$ (mean-fill faithfulness median exactly $0.00$; truncated restoration's zero-fill explodes to $4.36$), the tri-mask membership passes $3/21$, while the weighted circuit passes $17/21$ at the same median size --- and its $100$-step variant is the cheapest arm in the entire matrix ($18$s median discovery).
On this evidence, compact faithfulness is reached only by jointly optimised membership \emph{and} amplitudes; the top-$n$ rows are naive truncations and a re-targeted bisection prune would improve them, but no re-ranking closes a $0.00$-to-$1.08$ mean-fill gap at matched size.
The prior latent-endpoint recipe is also tested directly.
The hierarchical-attribution row replicates \citet{ge2024automatically}: gradient flow stopped during the backward at below-threshold nodes, errors detached, no optimisation and no intervention gating (the discovery class \citet{golimblevskaia2025circuit} also instantiates), adapted to our sites and rooted at the seed's pre-activation, with the threshold bisected per seed to matched size.
It passes $0/21$; its zero-fill median of $0.63$ hides a dead-or-overshoot bimodality (a single seed lands the zero fill in band, and per-seed values run from $0.00$ to $186$).
One part of their methodology does survive transfer to real-model intervention: hierarchical gating selects control-relevant nodes, posting the highest bare drive of the truncated arms ($0.86$ median against $0.56$--$0.66$).
No attribution recipe, however strengthened, buys compact faithfulness.
Third, and the reason no single number can rank methods, the \emph{home-turf effect}: a method wins the metric whose semantics it trains, observed six separate ways across this programme, most sharply in the no-zero-term amplitude control.
...
The replication extends the matrix into the prior work's own evaluation.
\citet{ge2024automatically} score circuits by attribution recovery inside their linearised graph, with no intervention; our first-order analogue is \emph{attribution-mass recovery}, the share of total activation-times-gradient mass a membership captures (a surrogate: the exact leaf-sum identity exists only in their substitute model).
Under it the ranking inverts: plain top-$n$ attribution leads (median signed recovery $0.76$), the replication follows ($0.72$), and the weighted circuit trails ($0.52$) while being the only arm that passes the intervention test.
The home-turf effect therefore reaches evaluation frameworks themselves, and the gap carries a finding: a circuit that reproduces the seed on held-out intervention while holding a third of the absolute attribution mass ($0.37$ median) shows that first-order attribution mass is not what faithfulness is made of.
...
The learned-mask pipeline is bit-deterministic at fixed settings: two identical runs agree to six decimal places with \emph{zero} membership flips.
Nothing below is therefore run-to-run noise, and tiny perturbations become controlled experiments on the objective itself.
Five probes give the same verdict.
\begin{itemize}
\item \textbf{Precision.} A float32 and a bfloat16 run differ only in rounding, yet share a membership Jaccard of $0.835$ at layer 8 ($16.5\%$ of members exchanged; $0.803$ at layer 10).
\texttt{free0} is a wash ($1.058$ vs $1.060$) and size agrees within $2\%$: a rounding-scale perturbation lands a different, equally good member set.
\item \textbf{Sparsity path.} Raising $\lambda$ swaps members rather than nesting them: containment between consecutive steps runs $0.49$--$0.80$.
The cleanest cell changes size by $4.6\%$ and held-out loss by $4.5\%$ while replacing $50.8\%$ of the membership.
\item \textbf{Time.} No gate ever freezes, under any schedule we tested.
A half-training snapshot is $57$--$72\%$ oversized and the second half of training removes $36$--$42\%$ of its members, so the stopping step is one more sampling axis.
...
\item \textbf{Budget.} At matched size, a short-budget high-price run and a long-budget low-price run share only $0.40$--$0.53$ of their members by Jaccard.
The two prunings also differ functionally: the long budget preferentially deletes drive-carrying members (\cref{sec:weighted}).
\end{itemize}
The organising claim: \textbf{aggregates are smooth; membership is not.}
Size falls monotonically with the sparsity price and faithfulness is stable under all five perturbations, but \emph{which} latents make the cut is identifiable only to the ${\sim}16$--$20\%$ level from 64 probe sequences.
A discovered membership is one sample from the family $\Phi_{\varepsilon}$ of \cref{sec:problem}, and these probes measure the family's breadth.
...
% RUNNING EXAMPLE 2026-08-10: members_triamp400/100.json overlap,
% experiments/032-running-example.
The running example makes the family concrete: its full-budget and quarter-budget weighted circuits both reconstruct the seed (held-out zero-fill $1.12$ and $1.13$), yet at $192$ and $250$ members they overlap at only Jaccard $0.60$: two samples from $\Phi_{\varepsilon}$, not one set discovered twice.
...
To measure where the latent endpoint parts from the behavioural one, we replicated sparse feature circuits as a first-class method on our own model and SAE bank, with the same evidence base and the same seeds, so the comparison is method against method, not architecture against architecture.
...
We read this as symmetry, not failure in either direction.
It is a scope result: ``which features support this output?'' and ``what makes this feature fire?'' are different questions whose answers share essentially no nodes, and because both directions share the same attribution estimator (\cref{sec:modes}), the separation isolates the endpoint and the membership rule rather than estimator quality.
One caveat bounds the reading: the behaviour-endpoint arm ran at its ceiling threshold without a held-out split, so this is a scope result, not a comparative one; a threshold sweep is required before any ranking claim.
...
Under the same 128-seed protocol, evidence base, and seed-intervention gates, two correlational baselines from the method catalogue (\cref{app:methods}) produced no accepted circuits at all, while ablation gradient accepted circuits for 121 of the same 128 seeds (\cref{tab:coact-methods}).
Sparse co-activation expansion follows the strongest co-activation edges outward from the seed; differential activation ranks latents by their positive-versus-negative activation contrast.
Correlational proposals do not merely score lower; they fail the intervention gates outright.
The strongest version of this challenger --- anchor co-activation selection granted the weighted circuit's own amplitude machinery --- revives on reconstruction but fails the joint calibration suite on every cell, and is treated in \cref{app:coact-amp}.
...
Two success criteria were colliding: \emph{interventional control}, which a compact circuit passes, available from the same discovery pass; and \emph{faithfulness}, whose cost is a measured quantity.
...
And faithfulness is a \emph{collective} property: every member is individually removable at the measurement floor, yet bulk removal by the members' own ranking craters the seed.
Any selection rule that thresholds individual contributions will therefore undercount it systematically, however its threshold is tuned.
...
The weighted-circuit results sharpen the thesis rather than soften it.
The cost is a measurement \emph{at stated amplitude semantics}: most of the collective tail is redundancy a rescaled survivor can carry (\cref{sec:weighted}), so a circuit's size is meaningful only alongside the amplitude convention it was measured under.
...
The intervention pair already shows it at the acceptance gate: $\phicf$ and $\phisup$ operationalise different claims, sufficiency to impose and necessity to sustain, and each method's accepted circuits look best on the metric it gates on.
The home-turf effect of \cref{sec:method-comparison} shows it in training.
...
The strongest quantitative form of the point is about membership itself: memberships trained under different floors share fewer latents than memberships trained for different objectives (Jaccard $0.13$ against $0.54$ at matched size); the counterfactual matters more than the target.
For weighted circuits the semantics reach the coefficients too: an untrained axis is not merely unmeasured but miscalibrated by one to three orders of magnitude (\cref{sec:weighted}).
...
Because every object is indexed by its seed, composition is available in principle: splice the circuit that explains an upstream latent into the circuits that use it, then re-validate by intervening at the earliest nodes and measuring propagation through intermediates.
The faithfulness results set its boundary condition: a composed chain inherits the faithful memberships of its links, so composition must manage membership, not merely concatenate stars.
...
\textbf{Protocol gaps.}
No strict held-out split is enforced (ablation gradient attributes and evaluates on the same probe set, and pruning optimises the acceptance metric), so reported scores are in-distribution intervention performance; the control/faithfulness magnitudes come from panels of three to sixteen seeds, pending the depth-stratified full-scale run; and the size-calibration exponent is recipe-bound, re-fit whenever the training dynamics change.
The weighted-circuit numbers come from a 22-seed depth-stratified panel with held-out probe contexts and $124$ amplitude-fitted null draws (\cref{sec:weighted}); still open there are a vacuity guard for the mean-fill denominator (one seed excluded by it), a per-kind account of the weak-drive MLP and bimodal attention hosts, and replication beyond this model and SAE bank.
...
A first replication on a public model with public Top-K dictionaries (\cref{sec:weighted}) covers six residual-stream seeds at two layers; a wider panel remains open.
The method is also scoped to Top-K dictionaries by construction --- its pre-activation objective exists because Top-K censors the activation --- and our exploratory runs on dense ReLU dictionaries sit outside that scope: circuits there are still faithful and necessary, but the fitted-amplitude null loses its discriminating power once a set is large enough to span the probe anchors, so that architecture needs its own null design before any claim is made on it.

\section{Conclusion}
% STEP 6 DONE (2026-07-30): matched to the six-contribution list.

This paper reframed circuit discovery around an internal endpoint, asking not \emph{which features caused this output} but \emph{what makes this feature fire}, and followed the reframing to its two criteria: signed one-hop dependencies validated by intervention, and the membership a latent needs before the circuit alone reproduces it.
...
The empirical lessons run in one direction.
Interventional control is concentrated while faithfulness is collective, position-local, and depth-scaled at natural amplitudes; amplitude freedom collapses its cost by one to two orders of magnitude, so circuit size is meaningful only with its amplitude convention.
Each method looks best under its own training semantics, so rankings are a matrix, not a column.
Membership is underdetermined while function is stable, so circuits are families, not sets.
Negative-context construction materially changes what is discovered.
None of it is recoverable from co-activation statistics, and none of it is reachable from behavioural endpoints.
...
At scale (32 seeds, paired) the on-manifold fill does not systematically shift normalised free faithfulness: it raises both $a_C$ and the floor, leaving the ratio a wash ($50$W/$46$L).
It therefore functions as a cleaner protocol rather than a score inflator; but at small memberships and tight fill budgets the two fills can disagree on \emph{rankings}, which is why the body's comparisons name their fill.
...
% data: experiments/005-floor-diagnostic/, 006-negctx-floor-grid/,
...
The faithfulness failure at depth is structural, not budget-limited; this is the result that motivated the membership-set object.
...
% data: experiments/003-eff-prune-test/
...
Edges never change membership or any evaluation score; they are descriptive.
But their statistics mirror the discovery mode: local attribution yields sparse shallow graphs (${\sim}9$ edges per node, depth ${\sim}4$) where path attribution yields dense deep ones (${\sim}40$ edges per node, depth ${\sim}10$), structure following the estimator's linearisation.
...
Faithfulness at depth, however, is error-carried: zeroing all upstream error terms leaves free faithfulness ${\approx}1.08$ at layer 2 but $0.005$ at layer 9.
Because raw and pruned memberships occupy every upstream site, selective error membership awaits discovery-side error attribution.
% data: experiments/004-error-node-mode/
...
The magnitude-bisection prune then composes with it: percentile selection raises the achievable faithfulness ceiling, the prune lowers size at fixed faithfulness, and the composition dominates on every gate seed.
On the mid-depth residual seed, top-$N$ selection cannot reach the target faithfulness at all, while the composed recipe reaches it at less than half the membership.
...
% data: experiments/010-lambda-calibration/ (README carries the full trail)
...
Annealing instead yields the tightest zero-ablation faithfulness in the programme ($0.002$--$0.10$ deviation from unity) at slightly worse fill scores, another home-turf demonstration.
...
% data: experiments/011-binarize-sweep/
...
\subsection{Co-Activation with Fitted Amplitudes: the Strongest Correlational Baseline}
\label{app:coact-amp}
% COACT ARC 2026-08-12: experiments/036-coact-home (home bank,
% best-vs-best triple floor), plus the same construction on the Llama
% skip-transcoder and GemmaScope banks (035-transcoder-compare,
% 037-gemmascope). Held-out protocol throughout; scored through the
% same code path as the discovered arms (selfcheck-certified on the Llama
% harness).
\Cref{sec:coact-results} shows correlational proposals failing the intervention gates outright; the fairest possible strengthening of that challenger is to hand it the weighted-circuit machinery itself.
Take the $n$ latents with the largest summed activation at the probe anchors (size-matched to the discovered weighted circuit), and score them twice through the identical pipeline: at natural values ($\alpha = 1$), and with amplitudes fitted by the same objective, floors, and budget as the discovered run.

At natural values the selection fails everywhere, and its failure is directional: $0.08$--$0.51$ at layer~2, but $7.6$--$9.1$ at layer~9 --- co-activation harvests excitation and misses the inhibitors that normally hold the seed down, so keeping only its picks \emph{overdrives} the target several-fold.
With fitted amplitudes it becomes the strongest baseline in the programme: zero-fill faithfulness reaches ${\sim}1$ at layer~2 on every seed.
The joint suite still separates it from the discovered circuits.
Under the production (triple-floor) recipe, best against best, the discovered weighted circuits hold both fill faithfulnesses on all five cells (zero-fill $0.92$--$1.06$, mean-fill $0.86$--$1.18$), while the co-activation sets hold both on none: at layer~2 the fitted amplitudes calibrate against one background and overshoot the other (mean-fill $1.30$--$2.47$, amplitudes reaching $6$--$10\times$), and at layer~9 no coefficient assignment reconciles the sign-blind selection with all three floors at once (zero-fill $3.09$ on one seed, $0.79$ on the other).
The same construction on two public dictionaries --- Top-K skip-transcoders for Llama-3.2-1B and GemmaScope JumpReLU SAEs for Gemma-2-2B --- reproduces the pattern: single scores come within reach, the joint criteria do not.
Reconstruction alone is therefore spoofable by a calibrated correlational set, and the revision to \cref{sec:coact-results} is precise: correlational \emph{selection} without coefficients fails outright, and with coefficients it fails calibration --- what it cannot buy at any coefficient setting is a membership whose contribution is floor-invariant, which is what the learned mask selects for.
