...
# LAMBDA x GAMMA grid for the dual floor.
#
# Two hypotheses to separate, from the 4-seed run:
#   OVER-PRUNING - dual carries TWO loss terms against ONE lambda*||m||_1, so
#     its effective sparsity roughly doubles. It landed 4.7x smaller than the
#     zero floor at L2 and 39.5x at L10, and free0 fell to 0.14 despite dual
#     optimising a zero-floor term directly. Lower lambda should fix it.
#   TERM CONFLICT - "be the delta from the negative baseline" fights "be
#     sufficient alone", and gamma=1 satisfies neither. Lower gamma should fix
#     it, and lambda alone should NOT.
# They predict different things, so the grid tells them apart: if size-matched
# dual (via lambda) recovers free0, it was pruning; if only lower gamma
# recovers it, the floors trade off.
#
# zero-floor is re-run per seed as the IN-RUN reference rather than quoted
# from the previous run - same seeds, same fixed eval negatives, one file.
# Extended DOWN two and a half decades. lambda 1e-5 still left dual BELOW
# the zero floor's size at L5 (6,367 vs 8,015), and at L10 dual was 39.5x
# too small - at ~3.8x more nodes per decade of lambda, closing that needs
# ~2.5 decades. The bottom end also probes where lambda STOPS biting: with
# a small enough per-latent price the L1 gradient falls under Adam's eps
# and nothing prunes at all (seen before at 327,680 kept members), so a
# flat size-vs-lambda tail is the signature of that floor, not of a
# converged solution.
# FOCUSED HEAD-TO-HEAD (2026-07-29, after the L5 lambda scan).
#
# The L5 scan put the knee at gamma 0.25 / lambda 3e-6: all three metrics
# within 0.036 of 1.0 on 19,843 nodes, beating lambda 1e-6 on BOTH k-sparse
# metrics with 2.6x fewer nodes. Lower lambda scored better only by keeping
# more - lambda 1e-7 held 198,282 nodes, 24x the zero floor, which is not a
# circuit. So the scan is done; what is missing is whether that knee holds on
# other seeds, and whether nudging both knobs down slightly does better.
#
# Three arms only, on THREE seeds, so the comparison is per-seed rather than
# a single-seed optimum: the in-run zero-floor reference, the L5 knee, and
# the new point.
ARMS = [
    ("zero-floor",          "zero", "store",  False, 1e-4, 1.0),
    ("dual g0.25 l3.0e-06", "dual", "random", True,  3e-6, 0.25),
    ("dual g0.20 l2.0e-06", "dual", "random", True,  2e-6, 0.20),
]
CANDIDATE = None
MATCHED = []
...
def base_state():
    ...
    disc.learned_mask.dual_floor_weight = 1.0
...
for seed_i, sc_idx, sl in todo:
    ...
    eval_sel = selector.select(
        ...
        preact_max_frac=PREACT_FRAC, posctx_reference=ref,
    ...
    def eval_row(label, circ, spec, secs):
        ...
        row = {"seed": seed_key, "seed_i": seed_i, "layer": layer, "kind": kind,
               ...
               "preact_max_frac": PREACT_FRAC, "preact_select": "cleanest",
               "l1_lambda": spec[3], "dual_floor_weight": spec[4], "posctx_ref": round(ref or 0.0, 4),
        ...
        print("  %-22s n=%8s free0=%-8s freeN=%-8s freeN_tk=%-8s freeM_tk=%-8s"
              % (label, format(n, ","), row["free0"], row["freeN"],
                 row["freeN_topk"], row["freeM_topk"]), flush=True)
        return n

    keep_circ, cand_n = {}, None
    for label, floor_src, neg_mode, pre, lam, gamma in ARMS:
        if (seed_key, label) in done and label not in {sl for sl, _ in MATCHED}:
            print("  skip (done) %s" % label, flush=True); continue
        try:
            base_state()
            disc.learned_mask.mask_floor_source = floor_src
            disc.learned_mask.l1_lambda = lam
            disc.learned_mask.dual_floor_weight = gamma
            ...
            n = eval_row(label, circ, (floor_src, neg_mode, pre, lam, gamma),
                         time.time() - t0)
            if label in {src for src, _ in MATCHED}:
                keep_circ[label] = circ
            if label == CANDIDATE:
                cand_n = n
            if label not in keep_circ:
                del circ
    ...
    for src_label, out_label in MATCHED:
        src = keep_circ.get(src_label)
        if src is None or not cand_n or (seed_key, out_label) in done:
            continue
        try:
            ranked = members_of(src)[:cand_n]
            wanted = {(st, i) for _, st, i in ranked}
            cm = Circuit(name=src.name)
            for u, node in src.nodes.items():
                f = node.feature_id
                if node.metadata.get("role") == "seed" or (
                        f is not None and ((f.layer, f.kind), int(f.index)) in wanted):
                    cm.nodes[u] = node
            cm.metadata = dict(src.metadata)
            eval_row(out_label, cm, ("truncated", src_label, False), 0.0)
        except Exception as exc:
            print("  %-24s FAILED %s: %s" % (out_label, type(exc).__name__, exc),
                  flush=True)
    keep_circ.clear()
    torch.cuda.empty_cache()
