...
#
# Three floor families x the negctx modes that feed them. "+rank" uses the
# rank-based pre-top-k filter (preact_select="cleanest": score a bounded pool
# pre-top-k, keep the quietest N). The previous run's "+pre" arms used an
# ABSOLUTE bar at 0.25 of the posctx reference, which Phase 1 later showed
# rejected NOTHING at L2/L5/L8 - those arms were duplicates of their
# unfiltered twins, so this run replaces them rather than extending them.
ARMS = [
    ("zero-floor",           "zero",   "store",  False),
    ("negctx/store",         "negctx", "store",  False),
    ("negctx/close",         "negctx", "close",  False),
    ("negctx/close+rank",    "negctx", "close",  True),
    ("negctx/random",        "negctx", "random", False),
    ("negctx/random+rank",   "negctx", "random", True),
    ("dual/store",           "dual",   "store",  False),
    ("dual/close",           "dual",   "close",  False),
    ("dual/close+rank",      "dual",   "close",  True),
    ("dual/random",          "dual",   "random", False),
    ("dual/random+rank",     "dual",   "random", True),
]
# Matched-node controls. The three families need not land at the same size
# under one lambda - masking to a floor removes less signal than masking to
# zero, so the L1 term buys a different amount of loss per unit m - and every
# fill-based metric is size-confounded. Both single-floor families are
# therefore ALSO evaluated truncated to the dual arm's node count, giving a
# three-way comparison at matched size.
CANDIDATE = "dual/random+rank"
MATCHED = [("zero-floor", "zero-floor@matched"),
           ("negctx/random+rank", "negctx/random+rank@matched")]
...
for seed_i, sc_idx, sl in todo:
    ...
    def eval_row(label, circ, spec, secs):
        ...
        row = {"seed": seed_key, "seed_i": seed_i, "layer": layer, "kind": kind,
               ...
               "preact_max_frac": PREACT_FRAC, "preact_select": "cleanest", "posctx_ref": round(ref or 0.0, 4),
