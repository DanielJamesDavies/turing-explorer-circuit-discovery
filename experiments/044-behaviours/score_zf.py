"""Re-score the fitted behaviour circuits under ZERO-FILL execution --
the frame the mask was actually optimised in (mask_floor_source=
"zero"). If EF jumps versus the mean-fill scores, the earlier numbers
measured a frame mismatch, not the circuits.

  PYTHONPATH=src python .../score_zf.py
"""
import json
import sys
...
for line in open(HERE / "behaviour_members.jsonl"):
    ...
    for site, d in r["alphas"].items():
        lyr, knd = site.split("/")
        keep[(int(lyr), knd)] = set(int(i) for i in d)
    members[r["cluster"]] = keep      # latest write wins (v2 run)


def metric(keep, tokens, anchors, targets):
...
for ck, keep in sorted(members.items()):
    idx = (assign == ck).nonzero(as_tuple=True)[0].tolist()
    if not idx:
        print("cluster %d not in current clustering, skipped" % ck)
        continue
    ...
    m_circ = metric(keep, pt_ho, pa_ho, tgt_ho)
    den = m_full - m_empty
    print("cluster %-3d ZERO-FILL: full %.3f | empty %.3f | circuit %.3f "
          "| EF %.3f" % (ck, m_full, m_empty, m_circ,
                         (m_circ - m_empty) / den if abs(den) > 1e-9 else -9))
