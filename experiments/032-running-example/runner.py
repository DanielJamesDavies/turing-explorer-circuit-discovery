"""RUNNING-EXAMPLE variant of the 2026-08-09 tri-amp panel runner:
one seed only — the paper's running example (L3 resid, comp 11,
latent 35381, "temperature as a thermodynamic state variable") — under
the IDENTICAL held-out protocol and arm specs (shallow-band config:
triple_w 0.10, lam100 4e-3), with no null draws, plus a per-arm
membership dump (members_<arm>.json) for the budget-churn comparison.
Produces the numbers for the running-example sentences in sec 5.2 /
sec 5.5. Derived from 029-panel/runner.py; scoring code
verbatim.
...
PANEL = [
    (11, "L3 resid", 1, 0.10, 4e-3, 0),
]
PINNED_SEEDS = {11: [35381]}
...
for comp_idx, band, n_seeds, triple_w, lam100, n_null in PANEL:
    ...
    for sl in seeds:
        ...
        for tag, fa, steps, lam in arm_specs:
            ...
            (HERE / ("members_%s.json" % tag)).write_text(json.dumps(
                [[l, kd, int(i), float(a)]
                 for (l, kd), d in alphas.items() for i, a in d.items()]))
            if tag == "triamp400":
                n_ref = r["n"]
