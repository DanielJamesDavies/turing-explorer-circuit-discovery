...
for sc_idx, sl in SEEDS:
    ...
    for mem, lab in ((mf, "MF"), (mf - inside, "MF-brakes"),
        ...
        rows.append(r)
        print("%-10s L%-2d %-5s %-10s n=%-6d (-%d) a=%7.3f/%7.3f free0=%-8s "
              "freeN=%-8s cf=%-7s sup=%s"
              % (seed_key, layer, kind, lab, r["n"], r["n_removed"],
                 r["a_circuit"], r["a_pos"], r["free0"], r["freeN_topk"],
                 r["cf"], r["sup"]), flush=True)
    torch.cuda.empty_cache()

with (HERE / "ablmask_minus_brakes.jsonl").open("w") as fh:
    for r in rows:
