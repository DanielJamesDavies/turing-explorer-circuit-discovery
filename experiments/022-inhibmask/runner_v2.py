...
BETAS = [2.0, 4.0]   # amplification factors for the mirror test
LAMBDAS = [float(x) for x in os.environ.get("LAMBDAS", "3e-4,1e-3,3e-3").split(",")]
...
class SetPatcher:
    """Set the given latents to given values (0.0 == silence); everything
    else natural. values: {(layer,kind): {idx: value}}."""
...
for sc_idx, sl in SEEDS:
    ...
    def act_under_values(values, tokens, argmax):
    ...
    def act_silencing(members, tokens, argmax):
        ...
        for site, idx in members:
            vals.setdefault(site, {})[idx] = 0.0
        return act_under_values(vals, tokens, argmax)

    def act_amplified(members, beta, tokens, argmax):
        """THE MIRROR TEST: fire the found brakes HARDER (beta x their own
        posctx value) and see whether the seed falls. phi-sup is the wrong
        instrument for this population — it sets inhibitors to their NEGCTX
        value, which for latents already present-and-braking on posctx
        LOWERS them (releasing the brake), which is why v1 read negative."""
        vals = {}
        for site, idx in members:
            base = float(pins_pos[site][idx]) if site in pins_pos else 0.0
            vals.setdefault(site, {})[idx] = beta * base
        return act_under_values(vals, tokens, argmax)
    ...
    for lam in LAMBDAS:
        ...
        def hook(step, ctx):
            ...
            drops = {}
            for b in BETAS:
                a_b = act_amplified(mem, b, pt_ev, pa_ev)
                drops["drop_b%g" % b] = round(
                    (a_pos_ev - a_b) / max(a_pos_ev, 1e-9), 4)
            # does the raise come from the R-inhibitor half or the R-activator
            # half of the membership? (attribution-sign agreement audit)
            mem_inh = [m for m in mem if m in ref_r_inh]
            mem_act = [m for m in mem if m in ref_r_act]
            r_inh_only = (round((act_silencing(mem_inh, pt_ev, pa_ev)
                                 - a_pos_ev) / max(a_pos_ev, 1e-9), 4)
                          if mem_inh else None)
            r_act_only = (round((act_silencing(mem_act, pt_ev, pa_ev)
                                 - a_pos_ev) / max(a_pos_ev, 1e-9), 4)
                          if mem_act else None)
            row = {"step": step, "n": len(mem), "raise_ho": raise_ho,
                   "raise_rand": raise_rand, "sup": sup_of(mem),
                   **drops, "raise_R_inh_only": r_inh_only,
                   "raise_R_act_only": r_act_only,
                   "n_R_inh": len(mem_inh), "n_R_act": len(mem_act),
        ...
        b = best
        print("  %-8s n_fin=%-6d | BEST step %-3s n=%-6s raise=%-7s rand=%-6s "
              "drop2=%-7s drop4=%-7s | raise(inh/act)=%s/%s R_inh=%-5s (%ss)"
              % (arm, len(scores), b.get("step"), b.get("n"),
                 b.get("raise_ho"), b.get("raise_rand"), b.get("drop_b2"),
                 b.get("drop_b4"), b.get("raise_R_inh_only"),
                 b.get("raise_R_act_only"), b.get("al_R_inh"), secs),
              flush=True)
