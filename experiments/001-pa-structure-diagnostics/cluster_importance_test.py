"""Cluster importance / drivers-vs-closure test (PA structure, test 6).

Hypothesis (the load-bearing "junk" is the closure backbone): position-SPECIFIC
latents form tight STABLE recipes (drivers); BROADLY-firing latents (fire at
many positions — what closure needs) can't be pinned to a recipe, so NMF dumps
them in UNSTABLE catch-all factors. If true: junk clusters hold the closure
mass AND their latents have high positional SPREAD.

Per seed (12), from split-half NMF (r=16) + greedy match, JUNK = match cos <0.9:
  - per-latent positional SPREAD = fraction of (b,t) positions it is captured at
  - stable vs junk: n latents, mass frac, mean spread
  - cross-cluster correlation: stability (match cos) vs mean spread, vs mass
  - free0 (production union eval): stable-only | junk-only | full
  - cumulative-by-stability curves: add clusters MOST-stable-first vs
    LEAST-stable-first; free0 at k = 4/8/12 clusters -> which order closes faster

Rows -> cluster_importance.jsonl.  PYTHONPATH=src python cluster_importance_test.py
"""
import json
import random
import time
...
EVAL_BS = 16
CUM_KS = (4, 8, 12)
OUT = Path(__file__).parent / "cluster_importance.jsonl"
    ...
    with OUT.open("a") as fh:
        for si, cand in enumerate(cands):
            ...
            try:
                ...
                cs = torch.tensor(greedy(HA, HB), device=device)          # [R] stability
                junk_clusters = (cs < JUNK_COS).nonzero(as_tuple=True)[0].tolist()

                MA = HA / HA.sum(0, keepdim=True).clamp_min(1e-12)          # [R, mc]
                home = MA.argmax(0)                                        # [mc]
                col_mass = ma_all[tc]                                      # [mc]
                spread = (Vf > 0).float().mean(dim=0)                      # [mc] frac positions active
                col_site = (tc // bank.d_sae); col_lat = (tc % bank.d_sae)

                # per-cluster stats
                clu_mass = torch.zeros(R, device=device); clu_mass.index_add_(0, home, col_mass)
                clu_n = torch.bincount(home, minlength=R).float()
                clu_spread = torch.zeros(R, device=device)
                clu_spread.index_add_(0, home, spread)
                clu_spread = clu_spread / clu_n.clamp_min(1)
                ...
                def keep_from(mask):
                    kd = defaultdict(set)
                    for j in mask.nonzero(as_tuple=True)[0].tolist():
                ...
                def free0(mask):
                    if abs(den) < 1e-9 or int(mask.sum()) == 0:
                        return None
                ...
                stable_mask = ~junk_mask
                # cumulative-by-stability free0 curves
                order_desc = torch.argsort(cs, descending=True).tolist()   # most stable first
                order_asc = torch.argsort(cs, descending=False).tolist()   # least stable first
                cum = {"stable_first": {}, "unstable_first": {}}
                for label, order in (("stable_first", order_desc), ("unstable_first", order_asc)):
                    for k in CUM_KS:
                        sel = set(order[:k])
                        mask = torch.zeros(mc, dtype=torch.bool, device=device)
                        for jc in sel:
                            mask |= (home == jc)
                        cum[label][k] = free0(mask)

                rec = {"comp": sc, "latent": sl, "layer": seed_layer, "kind": seed_kind, "m_cols": mc,
                       "n_junk_clusters": len(junk_clusters),
                       "stable_n": int(stable_mask.sum()), "junk_n": int(junk_mask.sum()),
                       "stable_mass_frac": round(float(col_mass[stable_mask].sum() / col_mass.sum()), 4),
                       "junk_mass_frac": round(float(col_mass[junk_mask].sum() / col_mass.sum()), 4),
                       "stable_spread": round(float(spread[stable_mask].mean()), 4),
                       "junk_spread": round(float(spread[junk_mask].mean()), 4),
                       "corr_stability_spread": round(corr(cs, clu_spread), 4),
                       "corr_stability_mass": round(corr(cs, clu_mass), 4),
                       "free0_full": free0(torch.ones(mc, dtype=torch.bool, device=device)),
                       "free0_stable_only": free0(stable_mask),
                       "free0_junk_only": free0(junk_mask),
                       "cum_stable_first": cum["stable_first"],
                       "cum_unstable_first": cum["unstable_first"],
                       "secs": round(time.time() - t_seed, 1)}
                fh.write(json.dumps(rec) + "\n"); fh.flush()
                print(f"[{si+1}/{len(cands)}] {sc}/{sl} L{seed_layer} {seed_kind}: "
                      f"spread stable/junk {rec['stable_spread']}/{rec['junk_spread']} "
                      f"corr(stab,spread)={rec['corr_stability_spread']} | "
                      f"free0 full/stable/junk {rec['free0_full']}/{rec['free0_stable_only']}/"
                      f"{rec['free0_junk_only']} | cum stable-first {rec['cum_stable_first']} "
                      f"unstable-first {rec['cum_unstable_first']} | {rec['secs']:.0f}s", flush=True)
