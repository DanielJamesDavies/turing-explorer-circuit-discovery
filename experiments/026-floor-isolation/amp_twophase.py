"""Two-phase test: are the amplitudes under-trained at 400 joint steps?

Phase 1 (standard tri-amp): gates + amplitudes trained jointly for 400
steps at the decision cell. Phase 2: FREEZE the membership (support=
mechanism, theta=+40, lambda=0 so nothing prunes, binarize off) and
train amplitudes ALONE — 400 and 800 steps — from init alpha=1 (the
engine has no psi warm-start; phase 2 is a refit on the fixed set,
which is also exactly the null's fitting procedure, so calibration
gains cannot be attributed to a different fitter).

If phase-2 alphas score like phase-1's, joint 400 was enough and the
amp-cf overshoot (1.17-1.50 at L9) is a property of the solution, not
under-training. If phase 2 (especially 800) moves ampF0/ampFMd closer
to 1 or shrinks the cf_amp overshoot, the two-phase schedule is a free
calibration upgrade at zero cost to circuit size.

Scoring is verbatim from the validated scripts: ampF0/ampFMd via
AmpCircuitPatcher + circuit_only_activation baselines (amp_null.py),
cf_amp via AmpInjectPatcher on live negctx (amp_cfsup.py).

  COMP_IDX=8  PYTHONPATH=src python experiments/026-floor-isolation/amp_twophase.py
  COMP_IDX=29 ...
"""
import json
import os
...
PHASE2_STEPS = tuple(int(s) for s in
                     os.environ.get("PHASE2_STEPS", "400,800").split(","))
...
SEEDS = SEEDS[:N_SEEDS]
print("L%d %s | seeds %s | cell %s | phase2 %s"
      % (LAYER, KIND, SEEDS, (CELL,), PHASE2_STEPS), flush=True)
...
fh = (HERE / ("amp_twophase_c%d.jsonl" % COMP_IDX)).open("a")
for sl in SEEDS:
    ...
    t0 = time.time()
    a1, st1 = discover()                          # phase 1: joint, cfg.steps
    members = [(s, i) for s, d in a1.items() for i in d]
    score(a1, st1, "joint%d" % cfg.steps, time.time() - t0)
    for p2 in PHASE2_STEPS:
        t0 = time.time()
        a2, st2 = discover(members=members, steps=p2)
        score(a2, st2, "amponly%d" % p2, time.time() - t0)
