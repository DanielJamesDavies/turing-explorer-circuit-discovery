"""Budget equivalence: can steps=100 at higher lambda reproduce the
steps=400 @ 1e-3 circuit at ~1/4 the compute?

Pruning pressure is roughly steps x lambda, so 100 steps @ 4e-3 is the
matched-budget arm; 2e-3 and 8e-3 bracket it. Each arm is a full joint
tri-amp discovery at the decision cell with (steps, lambda) overridden;
scoring verbatim from the validated patchers. Memberships are saved so
the circuits can be compared to the 400-step reference by Jaccard, not
just size.

  COMP_IDX=8 FAST_STEPS=100 LAMS=0.002,0.004,0.008 PYTHONPATH=src python experiments/026-floor-isolation/amp_fastlam.py
"""
import json
import os
import random
...
FAST_STEPS = int(os.environ.get("FAST_STEPS", 100))
LAMS = tuple(float(s) for s in
             os.environ.get("LAMS", "0.002,0.004,0.008").split(","))
...
SEEDS = SEEDS[:N_SEEDS]
print("L%d %s | seeds %s | cell %s | steps %d | lams %s"
      % (LAYER, KIND, SEEDS, (CELL,), FAST_STEPS, LAMS), flush=True)
...
fh = (HERE / ("amp_fastlam_c%d.jsonl" % COMP_IDX)).open("a")
for sl in SEEDS:
    ...
    def discover(members=None, steps=None, lam=None):
        floor, tw, cell_lam = CELL
        lam = cell_lam if lam is None else lam
    ...
    for lam in LAMS:
        t0 = time.time()
        a1, st1 = discover(steps=FAST_STEPS, lam=lam)
        tag = "s%d_lam%g" % (FAST_STEPS, lam)
        score(a1, st1, tag, time.time() - t0)
        mem = {"%d/%s" % (l, k): sorted(d) for (l, k), d in a1.items()}
        with (HERE / ("amp_fastlam_members_c%d.jsonl" % COMP_IDX)).open("a") as mf:
            mf.write(json.dumps({"latent": sl, "arm": tag, "members": mem}) + "\n")
