"""Step-count sweep for tri-amp discovery: evals, node counts and
wall-clock over the joint training budget.

steps is a SPARSITY DIAL as well as a convergence dial: pruning
pressure accumulates over steps (and the lr schedule stretches with
the budget), so n(steps) is part of the response, not a nuisance.
Each arm is a complete, independent joint discovery at the decision
cell with only `steps` changed; scoring is verbatim from the
validated patchers (ampF0/ampFMd via AmpCircuitPatcher, cf_amp via
AmpInjectPatcher).

  COMP_IDX=8  STEPS=50,100,200,400,800,1600 PYTHONPATH=src python experiments/026-floor-isolation/amp_stepsweep.py
  COMP_IDX=29 STEPS=100,200,400,800 ...
"""
import json
import os
import random
...
STEPS = tuple(int(s) for s in
              os.environ.get("STEPS", "50,100,200,400,800,1600").split(","))
...
SEEDS = SEEDS[:N_SEEDS]
print("L%d %s | seeds %s | cell %s | steps %s"
      % (LAYER, KIND, SEEDS, (CELL,), STEPS), flush=True)
...
fh = (HERE / ("amp_stepsweep_c%d.jsonl" % COMP_IDX)).open("a")
for sl in SEEDS:
    ...
    for nsteps in STEPS:
        t0 = time.time()
        a1, st1 = discover(steps=nsteps)
        score(a1, st1, "joint%d" % nsteps, time.time() - t0)
