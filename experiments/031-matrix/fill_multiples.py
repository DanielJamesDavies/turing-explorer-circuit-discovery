"""FILL: truncation multiples for the matrix arms (@x4/@x16/@x64),
appending to the same rows.jsonl. Copy of the definitive cross-method matrix — attribution arms on the panel's
...
for comp_idx, band, sl, n_ref in SEEDS:
    if (comp_idx, sl, "resto@x64") in done:
    ...
    for tag, mname, mode in ARMS:
        if (comp_idx, sl, tag + "@x64") in done:
            continue
        ...
        for m in (4, 16, 64):
            t2 = tag + "@x%d" % m
            if (comp_idx, sl, t2) in done:
                continue
            score_set(members_of(circuit, top_n=n_ref * m), t2, disc_secs)
        del circuit
