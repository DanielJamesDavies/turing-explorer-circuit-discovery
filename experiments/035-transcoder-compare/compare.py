...
def load_theirs():
    out = {}
    p = HERE / "theirs_llama_nodes.jsonl"
    if not p.exists():
        raise SystemExit("run theirs_llama.py first (needs ../../dev-notes/data/venv-ct)")
    for line in p.open():
        r = json.loads(line)
...
def load_ours():
    out = {}
    p = HERE / "ours_llama_members.jsonl"
    for line in p.open():
        r = json.loads(line)
        out[(r["layer"], r["latent"])] = r
    return out
...
def main():
    ...
    seeds = scan["seeds"]
    fh = (HERE / "compare_rows.jsonl").open("a")
