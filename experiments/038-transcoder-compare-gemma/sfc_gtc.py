"""SFC-STYLE ATTRIBUTION (external method E2) on the Gemma transcoder
...
def main():
    scan = torch.load(HERE / O.SCAN_NAME if hasattr(O, "SCAN_NAME") else
                      HERE / "scan_gtc.pt", weights_only=False)
    toks, seeds = scan["tokens"], scan["seeds"]
    sizes = {}
    rows_path = HERE / (O.ROWS_NAME if hasattr(O, "ROWS_NAME") else "ours_gtc_rows.jsonl")
    if rows_path.exists():
    ...
    for key in sorted(seeds):
        ...
        cap = {}
        hd = O.block(L).pre_feedforward_layernorm.register_forward_hook(
            lambda m, i, o: cap.__setitem__("f", O.features(L, o)))
        with torch.no_grad():
        ...
        for s0 in range(0, pos_tr.shape[0], BATCH):
            ...
            def read_hook(layer):
                def hook(mod, inp, out):
                    c = O.features(layer, out).detach()
                    d = torch.zeros_like(c, requires_grad=True)
                    codes[layer], deltas[layer] = c, d
                    return None
                return hook

            def write_hook(layer):
                def hook(mod, inp, out):
                    return out + deltas[layer].to(out.dtype) @ O.tc(layer)["W_dec"]
                return hook

            for l in UP:
                blk = O.block(l)
                handles.append(blk.pre_feedforward_layernorm
                               .register_forward_hook(read_hook(l)))
                handles.append(blk.post_feedforward_layernorm
                               .register_forward_hook(write_hook(l)))
            seedcap = {}
            handles.append(O.block(L).pre_feedforward_layernorm
