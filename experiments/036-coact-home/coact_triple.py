"""BEST vs BEST at depth: triple-floor tri-amp discovery against the
triple-floor coact baseline, on TuringLLM (k=128 / 40,960).

The dual-trained L9 comparison was inconclusive for a KNOWN reason: dual
fits overshoot ampFMd at depth on our own circuits too (1.42/2.36 —
the overshoot the triple floor was built to fix). So this rerun gives
BOTH arms the production floor: discovery = triple + free_amplitude +
lambda 1e-3; coact = the same anchor-mass selection, size-matched to the
triple-discovered circuit, alphas fitted under triple with lambda 0.
...
print("L%d %s | seeds %s | triple floor, best-vs-best"
      % (LAYER, KIND, SEEDS), flush=True)
...
fh = (HERE / ("coact_triple_c%d.jsonl" % COMP_IDX)).open("a")
for sl in SEEDS:
    ...
    # ARM 1: triple-floor tri-amp discovery (the production recipe)
    t0 = time.time()
    dsc, dprov = run_learned_mask(
    ...
    ak = dprov.get("amp_kept") or {}
    dalphas = {}
    for k, d in ak.items():
        lyr, knd = k.split("/")
        dalphas[(int(lyr), knd)] = {int(i): float(v) for i, v in d.items()}
    st = dprov.get("amp_stats") or {}
    score(dalphas, "disc_triple", time.time() - t0,
          {"alpha_p90": st.get("p90"), "alpha_max": st.get("max")})
    n_disc = sum(len(v) for v in dalphas.values())
    if not n_disc:
        print("[%d] empty triple discovery; skipping coact" % sl, flush=True)
        continue
