"""
Phase 7 — MlpTopCoactSparseExpansion circuit discovery algorithm tests.
...
def setup(mock_model, mock_sae_bank, monkeypatch):
    ...
    import circuit.discovery.top_coact_expansion.mlp_top_coact_sparse_expansion as mse_mod
    ...
    algo = MlpTopCoactSparseExpansion(
        inference        = inference,
...
class TestMlpNodeExpansion:
    ...
    def test_seed_node_is_mlp_with_seed_role(self, setup):
        ...
        assert seed_nodes[0].feature_id.kind == "mlp"
    ...
    def test_hop1_nodes_all_mlp(self, setup):
        ...
        assert len(hop1) > 0
        for node in hop1:
            assert node.feature_id.kind == "mlp", (
    ...
    def test_no_duplicate_nodes(self, setup):
        ...
        for node in circuit.nodes.values():
            key = (
                ...
                node.feature_id.kind,
                node.feature_id.index,
            )
    ...
    def test_passthrough_nodes_are_attn_or_resid(self, setup):
        ...
        for node in circuit.nodes.values():
            if node.metadata["role"] == "passthrough":
                assert node.feature_id.kind in ("attn", "resid"), (
...
class TestRejectionCases:
    ...
    def test_accepted_circuit_has_all_metadata_keys(self, setup):
        ...
        assert circuit.metadata["discovery_method"] == "mlp_top_coact_sparse_expansion"
